...
new_df_2 <- new_df_1 %>%
  mutate(reaction_time = new_df_1$response_time - new_df_1$stimulus_onset)
```
...
new_df_2$subject <- as.factor(new_df_2$subject)

new_df_3 <- new_df_2 %>%
  group_by(subject, congruency) %>%
  summarise(completed_trials = plyr::count(new_df_2$subject))
...
new_df_2$reaction_time <- as.double(new_df_2$reaction_time)

new_df_5 <- new_df_2 %>%
  filter(reaction_time <= 2000*(10^(-6)))
```
...
new_df_2$congruency <- as.factor(new_df_2$congruency)
...
new_df_7 <- new_df_2 %>%
  group_by(congruency) %>%
  dplyr::summarise(mean_react = mean(reaction_time),
                   std_err = describe(reaction_time)[[13]])
