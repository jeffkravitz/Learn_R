---
title: "Jeff's Journal"
output:
  html_document:
    toc: true
    toc_float: true
    collapsed: false
    number_sections: false
    toc_depth: 2
    #code_folding: hide
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(message=FALSE,warning=FALSE, cache=TRUE)
```

# Easy Problems

## 1. Simple math with numbers
```{r}
1+68
50-3
9/3
142*5
```

## 2. Put numbers into variables, do simple math
```{r}
jawbreakers = 10
cost = 25
jawbreakers * cost
cost/jawbreakers
```

## 3a. Place the numbers 1 to 100 separately into a variable using for loop
```{r}
ed <- length(100)
for (i in 1:100) {
  ed[i] <- i
}
print(ed)
```

## 3b. Place the numbers 1 to 100 separately into a variable using seq
```{r}
edd <- seq(1,100,1)
print(edd)
```

## 4. Find the sum of all the integer numbers from 1 to 100
```{r}
eddie <- sum(seq(1,100,1))
print(eddie)
```

## 5. Write a function to find the sum of all integers between any two values
```{r}
int_sum <- function(x, y) {
  temp = 0
  jeff_sum = 0
  if (x > y){
    temp = x
    x = y
    y = temp
  }
  for (i in x:y){
    jeff_sum = jeff_sum + i
  }
  return(jeff_sum)
  print(jeff_sum)
}
```

## 6. List all of the odd numbers from 1 to 100
```{r}
odd_sum <- sum(seq(1,100,2))
print(odd_sum)
```

## 7. List all of the prime numbers from 1 to 1000
```{r}
prime_numbers <- c()
for (i in 3:1000) {
  prime = TRUE
  for (j in 2:(i-1)) {
    if (i %% j == 0) {
      prime = FALSE
    }
  }
  if (prime == TRUE) {
    prime_numbers <- c(prime_numbers,i)
  }
}
print(prime_numbers)
```


## 8. Generate 100 random numbers
```{r}
sample(1:100, 100, replace=T)
```

## 9. Generate 100 random numbers within a specific range
```{r}
sample(1:100, 100, replace=T)
```

## 10a. Write your own functions to give descriptive  
statistics for a vector variable: Mean
```{r}
jeff_mean <- function(x) {
  jeff_sum <- 0
  for (i in 1:(length(x))) {
    jeff_sum <- jeff_sum + (x[i]/length(x))
  }
  return(jeff_sum)
  print(jeff_sum)
}
```

## <span style="color:red">*10b. Write your own functions to give descriptive statistics for a vector variable: Mode*</span>
